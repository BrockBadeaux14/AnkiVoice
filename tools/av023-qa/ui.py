#!/usr/bin/env python3
"""Bounded AV-023 UI captures on the dedicated synthetic AVD; no review actions."""
import argparse
import json
import re
import subprocess
...
EVIDENCE = Path(__file__).resolve().parents[2] / 'docs/testing/av023/evidence'
...
def main():
    ...
    if adb('emu', 'avd', 'name').decode().splitlines()[0] != 'AnkiVoice_AV023':
        raise SystemExit('Refusing any AVD other than AnkiVoice_AV023')
    xml = tree()
