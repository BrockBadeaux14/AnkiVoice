 ...
 *
 * AV-050: this screen carries **no sign of the grading mode**. AV-047's setting still
 * exists, still changes what is written, and is still shown and explained on the setup
 * screen; a running session simply never names it. Nothing here — no banner, no countdown,
 * no **Keep it manual**, no second variant of the Confirm sentence — tells the learner
 * which mode this session opened in, and a guard test fails if any text this screen can
 * produce says "Automatic grading". The one thing that is still shown is a rating that was
 * left **unwritten**: silence about a review that does not exist would be a lie.
 *
 * AV-050 also removes the two taps between a card and the learner speaking. The prompt is
 * spoken when the card is offered, and the microphone opens when that playback settles.
 * **Play prompt** and **Start answer** stay here as touch controls: they are offered
 * whenever they apply, for a failed automatic open and for a learner who wants to start
 * early.
 */
@Composable
    ...
    Card(
        ...
        Column(Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            if (StudyControl.CONFIRM in controls || StudyControl.CHANGE in controls) {
                ...
                Text(
                    "Confirm is the one control on this screen that writes a review. Everything else " +
                        "leaves your collection alone.",
                    style = MaterialTheme.typography.bodySmall,
                )
            }
        }
