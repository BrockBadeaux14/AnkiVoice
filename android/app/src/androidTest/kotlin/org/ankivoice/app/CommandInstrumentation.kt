...
class CommandInstrumentation : Instrumentation() {
    ...
    override fun onStart() {
        ...
        try {
            ...
            output.put("contextRule", contextRule(open, router, card))
            if (open.halted) open.resume()
            output.put("voice", voiceSweep(open, router, language))
     ...
     */
    private fun contextRule(session: ReviewSession, router: CommandRouter, card: ScheduledCard): JSONObject {
        ...
        if (interactive) {
            val heard = session.listen()
            result.put("capture", describe(heard))
     ...
     */
    private fun voiceSweep(session: ReviewSession, router: CommandRouter, language: String): JSONArray {
        val log = JSONArray()
        if (!interactive) {
            log.put(
                JSONObject()
        ...
        for (command in wanted) {
            // A fresh, thinking turn for each attempt, so one command cannot leave the next
